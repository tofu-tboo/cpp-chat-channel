...
namespace ALC_Traits {
    ...
    template<typename T> concept HasIterators = requires(T t) { t.begin(); t.end(); };
...
public:
    ...
    template <typename Key>
    bool find(const Key& key) const requires ALC_Traits::HasKey<STL>;
    bool find(const value_type& item) const requires (!ALC_Traits::HasKey<STL> && ALC_Traits::HasIterators<STL>);
