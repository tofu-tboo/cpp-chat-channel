...
#define alloc_json_new(name, ...)             json name = NULL; alloc_json(name, __VA_ARGS__)
#define alloc_json(name, ...)                 name = json_pack(__VA_ARGS__); if (name)
#define alloc_fail                            else
// #define __FREE_JSON(name)                       free_json(name);
// #define __FREE_JSONS(...)                       free_jsons(CNT_ARGS(__VA_ARGS__), __VA_ARGS__)
#define unpack_json(target, ...)              if (json_unpack((target).get(), __VA_ARGS__) >= 0)
#define unpack_fail                           else
